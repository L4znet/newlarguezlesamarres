...
class BookingRepositorySupabase implements BookingRepository {
     async getTenantBookings(userId: string): Promise<BookingEntity[] | undefined> {
          const { data: bookingData, error: bookingError } = await supabase.from("bookings").select(`*, ...offers!inner(offer_title:title, offer_description:description, offer_price:price,offer_frequency:frequency ,...boats(boat_name,boat_images (id,caption,url)))`).eq("user_id", userId)
